...
To assign a certain amount of fields a certain crop type
```{r}
crop_type <- function(land, agents = 11) { # agents = 1 (manager) + number of stakeholders
  xdim <- dim(land)[1]
  ydim <- dim(land)[2]
...
file_path <- "C:/Users/imket/Documents/GitHub/European_hamster_GMSE/Output/test_landscape.csv" #Give the simulation an appropriate name
write.csv(combined_results, file = file_path, row.names = FALSE)

image(x = sim_old$LAND[,,1], col = c("blue", "green", "yellow"), xaxt = "n", yaxt = "n")
legend("topright", legend = c("Crop Type 1", "Crop Type 2", "Crop Type 3"),
       fill = c("blue", "green", "yellow"), cex = 0.8) #to show crop type distribution in the landscape
