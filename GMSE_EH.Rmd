...
crop_type <- function(land, stakeholders = 100, types = 3){ # stakeholders: number of stakeholders/farms, types = number of possible crop types
  xdim <- dim(land)[1];
  ydim <- dim(land)[2];
  for(i in 1:xdim){   
  for(j in 1:ydim){
...
Allemaal startende van 250 individuen, aantal stakeholders onbelangrijk hier omdat er geen acties ondernomen worden.

### crop type 3, 2 litters
...
file_path <- "C:/Users/imket/Documents/GitHub/European_hamster_GMSE/100iterations_type3_startN30.csv"
...
# Initialise the first output
sim_old   <- gmse_apply(stakeholders    = 100, # should be the same as in function crop_type
                        res_movement    = 0, 
                        remove_pr       = 1 - 0.976083968, 
                        lambda          = 0, 
                        res_death_type  = 1,
