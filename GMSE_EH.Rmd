...
crop_hamster <- function(land, res){     
    ...
    for(i in 1:N) { 
        ...
        } else if (crop == 3 && next_time %in% c(JAN)) { 
          res[i, 9] <- 1-0.966
        } else if (crop == 3 && next_time %in% c(FEB)) { 
          res[i, 9] <- 1-0.96
        } else if (crop == 3 && next_time %in% c(MAR)) { 
          res[i, 9] <- 1-0.919
        } else if (crop == 3 && next_time %in% c(APR)) { 
          res[i, 9] <- 1-0.953
        } else if (crop == 3 && next_time %in% c(MAY)) { 
          res[i, 9] <- 1-0.859
        } else if (crop == 3 && next_time %in% c(JUN)) { 
          res[i, 10] <- 1.28
          res[i, 9] <- 1-0.827
        } else if (crop == 3 && next_time %in% c(JUL)) { 
          res[i, 10] <- 1.28
          res[i, 9] <- 1-0.839
        } else if (crop == 3 && next_time %in% c(AUG)) { 
          res[i, 10] <- 1.28
          res[i, 9] <- 1-0.838
        } else if (crop == 3 && next_time %in% c(SEP)) { 
          res[i, 9] <- 1-0.91
        } else if (crop == 3 && next_time %in% c(OCT)) { 
          res[i, 9] <- 1-0.897
        } else if (crop == 3 && next_time %in% c(NOV)) { 
          res[i, 9] <- 1-0.923
        } else if (crop == 3 && next_time %in% c(DEC)) { 
          res[i, 9] <- 1-0.972
        } else {
...
# Initialise the first output
sim_old   <- gmse_apply(stakeholders    = 10, 
                        res_movement    = 100, 
                        res_move_type   = 0,
                        remove_pr       = 1 - 0.976083968, 
                        lambda          = 0, 
...
# Crop type plot
# Adjust plot margins to make space for the legend
output_folder<- "C:/Users/lucp13135/Documents/Simulations/Simulation 12b"
filename_crop <- file.path(output_folder, paste0(timestamp, ".png"))
png(filename_crop, width = 1000, height = 800)
par(mar = c(5, 4, 4, 8) + 0.1)
image(z = sim_old$LAND[,,1], col = c("#FF6961", "#EBD657", "#92C37F"), xaxt = "n", yaxt = "n", 
...
file_path <- "C:/Users/lucp13135/Documents/Simulations/Simulation 12b/Simulation12b_100it_250startN.csv" #Give the simulation an appropriate name
write.csv(combined_results, file = file_path, row.names = FALSE)
