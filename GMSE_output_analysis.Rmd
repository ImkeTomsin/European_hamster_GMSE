...
sim_11 <- read.csv("C:/Users/imket/Documents/GitHub/European_hamster_GMSE/Output/simulation11_100it_250startN.csv", header=TRUE,sep=",")
#Add starting population size (time = 0, N = 15 or 250) to each iteration
new_rows <- sim_11 %>%
  group_by(rep) %>%
  slice(1) %>%
...
sim_12 <- read.csv("C:/Users/imket/Documents/GitHub/European_hamster_GMSE/Output/simulation12_100it_250startN.csv", header=TRUE,sep=",")
#Add starting population size (time = 0, N = 15 or 250) to each iteration
new_rows <- sim_12 %>%
  group_by(rep) %>%
  slice(1) %>%
...
sim_13 <- read.csv("C:/Users/lucp13135/Documents/Simulations/Simulation 13/Simulation13_100it_250startN.csv", header=TRUE,sep=",")
#Add starting population size (time = 0, N = 15 or 250) to each iteration
new_rows <- sim_13 %>%
  group_by(rep) %>%
  slice(1) %>%
  ...
  select(rep, Time, Pop_size)
# Bind the new rows to the original data
sim_13 <- bind_rows(sim_13, new_rows)


sim_14 <- read.csv("C:/Users/lucp13135/Documents/Simulations/Simulation 14/Simulation14_100it_250startN.csv", header=TRUE,sep=",")
#Add starting population size (time = 0, N = 15 or 250) to each iteration
new_rows <- sim_14 %>%
  group_by(rep) %>%
  slice(1) %>%
  ...
  select(rep, Time, Pop_size)
# Bind the new rows to the original data
sim_14 <- bind_rows(sim_14, new_rows)
...
plot_13<-generate_plot(sim_13,"Scenario 13")
plot_14<-generate_plot(sim_14,"Scenario 14")
